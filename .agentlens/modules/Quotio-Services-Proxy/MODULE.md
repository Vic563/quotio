# Module: Quotio/Services/Proxy

[← Back to INDEX](../../INDEX.md)

**Type:** implicit | **Files:** 5

## Files

| File | Lines | Large |
| ---- | ----- | ----- |
| `Quotio/Services/Proxy/CLIProxyManager.swift` | 2121 | 📊 |
| `Quotio/Services/Proxy/EffortTranslator.swift` | 89 |  |
| `Quotio/Services/Proxy/FallbackFormatConverter.swift` | 109 |  |
| `Quotio/Services/Proxy/ProxyBridge.swift` | 1127 | 📊 |
| `Quotio/Services/Proxy/ProxyStorageManager.swift` | 402 |  |

## Documentation

- [outline.md](outline.md) - Symbol maps for large files
- [memory.md](memory.md) - Warnings and TODOs

---

_No import relationships detected._